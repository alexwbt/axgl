...
public:
  ...
  void tick() const
  {
    for (const auto& comp : components_)
      if (!comp->is_disabled() && !comp->is_static())
        comp->parent_tick(parent_);
  }
  ...
  void remove(const axgl::ptr_t<axgl::Component>& component) override
  {
