...
public:
  ...
  public:
    virtual ~CameraMode() {}
    virtual void bind_inputs(interface::InputService& input_service) = 0;
    virtual void unbind_inputs(interface::InputService& input_service) = 0;
...
class FreeCameraMode : public CameraService::CameraMode
{
private:
...
public:
  ...
  void bind_inputs(interface::InputService& input_service) override
  {
  ...
  void unbind_inputs(interface::InputService& input_service) override
  {
  ...
  {
    if (pointer_->tick > 0 && pointer_->delta.x != 0)
      camera.yaw -= pointer_->delta.x * view_movement_speed_;

    if (pointer_->tick > 0 && pointer_->delta.y != 0)
...
class Free2DCameraMode : public CameraService::CameraMode
{
private:
  std::shared_ptr<interface::Input> up_;
  std::shared_ptr<interface::Input> down_;
