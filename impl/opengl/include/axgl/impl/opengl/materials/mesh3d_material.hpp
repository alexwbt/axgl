...
public:
  ...
  void add_texture(
    ...
    const axgl::ptr_t<axgl::Texture> texture) override
  {
...
private:
  void use_lights(
    const std::span<const impl::opengl::renderer::LightContext>& lights) const
  {
  ...
  void use_texture(
    ...
    const axgl::ptr_t<impl::opengl::Texture>& texture) const
  {
    if (!texture)
    {
